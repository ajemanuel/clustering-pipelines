...
def load_intan_input_channels():
	...
	channel_ids = set(p.findall(' '.join(sorted(glob.glob('*.di')))))
	#load in each channel and concatenate.
	digital_inputs = {}
	for ch in channel_ids:
		files = sorted(glob.glob('*chan' + ch + '.di'))
		for f in files:
	...
	channel_ids = set(p.findall(' '.join(sorted(glob.glob('*.ai')))))
	#load in each channel and concatenate.
	analog_inputs = {}
	for ch in channel_ids:
		files = sorted(glob.glob('*chan' + ch + '.ai'))
		for f in files:
